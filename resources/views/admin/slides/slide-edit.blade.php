        ...
        <div class="main-content-wrap">
            ...
            <div class="wg-box">
                <form class="form-new-product form-style-1" method="POST"
                    action="{{ route('admin.slide.update', $slide->id) }}" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    <fieldset class="name">
                        <div class="body-title">Tagline <span class="tf-color-1">*</span></div>
                        <input class="flex-grow @error('tagline') is-invalid @enderror" type="text" placeholder="Tagline"
                            name="tagline" tabindex="0" value="{{ $slide->tagline }}" aria-required="true">
                        @error('tagline')
                    ...
                    <fieldset class="name">
                        ...
                        <input class="flex-grow @error('title') is-invalid @enderror" type="text" placeholder="Title"
                            name="title" tabindex="0" value="{{ $slide->title }}" aria-required="true">
                        @error('title')
                            <div class="text-danger">{{ $message }}</div>
                        @enderror
                    </fieldset>
                    <fieldset class="name">
                        ...
                        <input class="flex-grow @error('subtitle') is-invalid @enderror" type="text"
                            placeholder="Subtitle" name="subtitle" tabindex="0" value="{{ $slide->subtitle }}"
                            aria-required="true">
                        @error('subtitle')
                            <div class="text-danger">{{ $message }}</div>
                        @enderror
                    ...
                    <fieldset class="name">
                        ...
                        <input class="flex-grow @error('link') is-invalid @enderror" type="text" placeholder="Link"
                            name="link" tabindex="0" value="{{ $slide->link }}" aria-required="true">
                        @error('link')
                            <div class="text-danger">{{ $message }}</div>
                        @enderror
                    </fieldset>
